...
def train_all_models(config: dict[str, Any], use_gpu: bool, device: torch.device, name: str,
    ...
    for cut in range(2):
        for mix in range(2):
            ...
            new_name: str = f'{name}-{cut}{mix}'
            test_loss: float
            test_acc: float

            test_loss, test_acc = train_model(
                bool(cut), bool(mix), train_loader, valid_loader, test_loader, config['stages'],
                config['optimizer'], config['scheduler'], config['max_epoch'], device,
                new_name, root_path
            )
...
def train_model(cut: bool, mix: bool, train_loader: DataLoader, valid_loader: DataLoader,
    ...
    result: tuple[float, float] = trainer.train(train_loader, valid_loader, test_loader,
                                                BatchAugmentation(cut, mix), max_epoch,
                                                logger, writer)
