...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str]):
    ...
    dataset_test = DatasetBase.from_name(dataset_name)("./data_test", DatasetBase.Mode.EVAL, Config.IMAGE_MIN_SIDE, Config.IMAGE_MAX_SIDE)
    ...
    dataloader_test = DataLoader(dataset_test, batch_size=1,shuffle=False, num_workers=8, pin_memory=True)
    evaluator = Evaluator(dataset_test, "./data_test", "./test_result")
    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if step % num_steps_to_display == 0:
                ...
                # print(f'[Step {step}] Avg. Loss = {avg_loss:.6f},anchor.obj. Loss = {anchor_objectness_loss:.6f},anchor.trans. Loss = {anchor_transformer_loss:.6f},prop.class. Loss = {proposal_class_loss:.6f},prop.trans. Loss = {proposal_transformer_loss:.6f}, Learning Rate = {lr:.8f} ({samples_per_sec:.2f} samples/sec; ETA {eta:.1f} hrs)')
                # print(f'[Step {step}] Test Avg. Loss = {test_loss[0]:.6f},Test anchor.obj. Loss = {anchor_objectness_loss_test:.6f},Test anchor.trans. Loss = {anchor_transformer_loss_test:.6f},Test prop.class. Loss = {proposal_class_loss_test:.6f},Test prop.trans. Loss = {proposal_transformer_loss_test:.6f}, Learning Rate = {lr:.8f} ({samples_per_sec:.2f} samples/sec; ETA {eta:.1f} hrs)')
                Log.i(f'[Step {step}] Avg. Loss = {avg_loss:.6f},anchor.obj. Loss = {anchor_objectness_loss:.6f},anchor.trans. Loss = {anchor_transformer_loss:.6f},prop.class. Loss = {proposal_class_loss:.6f},prop.trans. Loss = {proposal_transformer_loss:.6f}, Learning Rate = {lr:.8f} ({samples_per_sec:.2f} samples/sec; ETA {eta:.1f} hrs)')
                # Log.i(f'[Step {step}] Test Avg. Loss = {test_loss[0]:.6f},Test anchor.obj. Loss = {anchor_objectness_loss_test:.6f},Test anchor.trans. Loss = {anchor_transformer_loss_test:.6f},Test prop.class. Loss = {proposal_class_loss_test:.6f},Test prop.trans. Loss = {proposal_transformer_loss_test:.6f}, Learning Rate = {lr:.8f} ({samples_per_sec:.2f} samples/sec; ETA {eta:.1f} hrs)')
                if step % 5000 ==0:
